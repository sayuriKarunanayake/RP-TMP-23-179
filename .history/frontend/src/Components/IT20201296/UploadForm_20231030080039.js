...
function UploadForm({ onImageUpload, onVideoUpload, verificationResult, headPosePercentage, blinkCount, onVerifyClick }) {
  ...
  const phoneRegex = /^\d{10}$/; // Regular expression for 10-digit phone number validation
   
  const handleSubmit = (e) => {
    e.preventDefault();
   
    const emailInput = document.getElementById("email").value; // Assuming you have an input field with id "email"
    const phoneInput = document.getElementById("phone").value; // Assuming you have an input field with id "phone"
    ...
    // If both email and phone number are valid, proceed with the axios post request
    axios
      .post("http://localhost:8070/addApplications", newApply)
      .then(() => {
